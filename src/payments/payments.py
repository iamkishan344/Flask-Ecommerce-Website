...
# from src.products.routes import  Apparel
...
def get_order():
    if current_user.is_authenticated:
        ...
        try:
            order = CustomerOrder(
                invoice=invoice, customer_id=customer_id, orders=session['cart'])
            db.session.add(order)
            db.session.commit()
            session.pop('cart')
...
def orders(invoice):
    ...
    return render_template('customer/order.html', invoice=invoice, tax=tax,
                           subTotal=subTotal, grandTotal=grandTotal, customer=customer, orders=orders, categories=categories())
...
@login_required
def thanks():
    return render_template('customer/thank.html', categories=categories())
