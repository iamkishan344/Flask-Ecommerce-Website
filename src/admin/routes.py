...
from src.products.models import Addproduct, Category
...
def dashboard():
    if is_admin():
        ...
        # return render_template('products/empty_cart.html')
        return render_template('admin/dashboard.html', title="Admin Page", products=products,admins=admins)
...
def update_orders(id):
    if is_admin() is False:
        return regiest_error_handler(404, page_not_found)
...
def delete_admin_user(id):
    ...
    if request.method == "POST":
        ...
        flash(
            f'The user {user.name} was deleted from your database', 'success')
        db.session.commit()
